...
const hasProfanity=s=>{if(!s)return false;const w=String(s).toLowerCase();return PROFANITY_PATTERNS.some(re=>re.test(w))};
...
function PerkLikelihoodSummary({perks}){
const rows=useMemo(()=>{
const now=Date.now();
const byCat={};
CATS.forEach(c=>{byCat[c.key]={...c,yes:0,no:0,yesW:0,noW:0,samples:0,lastReport:0}});
(perks||[]).forEach(p=>{
const cat=byCat[p.category];if(!cat)return;
const signalTs=ts(p.stay_date||p.created_at)||0;
const reportTs=ts(p.created_at)||0;
const ageDays=signalTs?Math.max(0,(now-signalTs)/86400000):730;
const recency=ageDays<=180?1.2:ageDays<=365?1:ageDays<=730?0.85:0.7;
const voteDelta=(p.upvotes||0)-(p.downvotes||0);
const voteBoost=1+Math.max(-0.2,Math.min(0.35,voteDelta*0.03));
const w=Math.max(0.3,recency*voteBoost);
const outcome=perkOutcome(p);
cat.samples+=1;
cat.lastReport=Math.max(cat.lastReport,reportTs);
if(outcome==="not_received"){cat.no+=1;cat.noW+=w}else{cat.yes+=1;cat.yesW+=w}
});
return CATS.map(c=>{
const x=byCat[c.key];
const totalW=x.yesW+x.noW;
const score=totalW?Math.round((x.yesW/totalW)*100):null;
const confidence=x.samples>=8?"High confidence":x.samples>=4?"Medium confidence":x.samples>0?"Low confidence":"No data";
return{...x,score,confidence,description:PERK_CATEGORY_DESC[c.key]||"Community reports for this category."}
});
},[perks]);
return<div style={{background:"#fff",borderRadius:10,padding:20,border:"1px solid #e2e8f0",marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}><h3 style={{fontSize:16,fontWeight:700,color:"#0f172a",fontFamily:FF,margin:0}}>Perk Likelihood Snapshot</h3><span style={{fontSize:10,color:"#64748b",fontFamily:FF,textTransform:"uppercase",letterSpacing:1}}>0-100 likelihood from UGC</span></div><p style={{fontSize:12,color:"#64748b",fontFamily:FF,margin:"0 0 14px"}}>100 means reports in this category mostly said "received". 0 means mostly "did not receive". Gray means no data yet.</p><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:8}}>{rows.map(r=>{const hasData=r.score!==null;const color=hasData?likelihoodColor(r.score):"#94a3b8";const bg=hasData?likelihoodBg(r.score):"#f8fafc";const dataLine=hasData?`${r.yes} of ${r.samples} reports said received`:"No community reports yet";return<div key={r.key} style={{background:bg,border:"1px solid #e2e8f0",borderRadius:8,padding:"9px 10px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:6,marginBottom:4}}><span style={{fontSize:11,fontWeight:700,color:"#0f172a",fontFamily:FF,display:"inline-flex",alignItems:"center",gap:5}}><span>{r.icon}</span>{r.label}</span><span style={{fontSize:11,fontWeight:700,color:color,fontFamily:FF}}>{hasData?`${r.score}`:"—"}</span></div><div style={{fontSize:10,color:"#64748b",fontFamily:FF,lineHeight:1.35,marginBottom:6}}>{r.description}</div><div style={{height:7,borderRadius:999,background:"#e2e8f0",overflow:"hidden",marginBottom:6}}><div style={{height:"100%",width:hasData?`${r.score}%`:"0%",background:hasData?`linear-gradient(90deg,${color}66,${color})`:"#cbd5e1",transition:"width 0.3s ease"}}/></div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}><span style={{fontSize:10,color:"#64748b",fontFamily:FF}}>{hasData?likelihoodLabel(r.score):"No reports yet"}</span><span style={{fontSize:9,color:"#94a3b8",fontFamily:FF}}>{hasData?`Yes ${r.yes} · No ${r.no}`:""}</span></div><div style={{fontSize:9,color:"#64748b",fontFamily:FF,marginTop:4}}>{dataLine}</div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginTop:4}}><span style={{fontSize:9,color:"#94a3b8",fontFamily:FF}}>{r.confidence}</span><span style={{fontSize:9,color:"#94a3b8",fontFamily:FF}}>{r.lastReport?`Updated ${ta(r.lastReport)}`:""}</span></div></div>})}</div></div>}
...
<div style={{fontSize:11,color:"#64748b",fontFamily:FF,display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:14,paddingTop:10,borderTop:"1px solid #f1f5f9",gap:8}}><div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}><span><span style={{color:"#0f172a",fontWeight:700,fontSize:14}}>{c}</span> reports</span>{h.room_count&&<span style={{fontSize:10,color:"#94a3b8",whiteSpace:"nowrap"}}>{h.room_count} rooms</span>}</div>{user&&onToggleFollow&&<button onClick={e=>{e.stopPropagation();onToggleFollow(h.id,followed)}} disabled={busy} style={{background:followed?"#e2e8f0":"#f8fafc",border:"1px solid #e2e8f0",borderRadius:4,padding:"2px 6px",fontSize:9,fontWeight:700,color:followed?"#334155":"#475569",fontFamily:FF,cursor:"pointer",opacity:busy?0.65:1,whiteSpace:"nowrap"}}>{followed?"Following":"Follow"}</button>}</div>
</div>})}
</div></div>}
...
if(isOwn){const{data:np,error:npErr}=await supabase.from("user_notification_prefs").select("*").eq("user_id",userId).maybeSingle();if(!npErr&&np){setDigestEnabled(np.monthly_digest_enabled!==false)}else if(npErr&&!String(npErr.message||"").includes("user_notification_prefs"))console.error(npErr)}
...
<div style={{flex:"1 1 220px"}}><label style={LS}>Monthly Email Digest</label><select value={digestEnabled?"enabled":"disabled"} onChange={e=>setDigestEnabled(e.target.value==="enabled")} style={IS}><option value="enabled">Enabled</option><option value="disabled">Disabled</option></select><div style={{fontSize:11,color:"#94a3b8",fontFamily:FF,marginTop:6}}>Sent once per month with top perk highlights from the past 30 days.</div></div>
</div>}
...
useEffect(()=>{setPerkCategoryFilter([])},[hotel.id]);
...
load()};
const tierUp={ambassador:["ambassador","titanium","platinum","gold","silver"],titanium:["titanium","platinum","gold","silver"],platinum:["platinum","gold","silver"],gold:["gold","silver"],silver:["silver"]};
const filteredPerks=perkCategoryFilter.length?perks.filter(p=>perkCategoryFilter.includes(p.category)):perks;
const byTierAll={};TIERS.forEach(t=>{const show=tierUp[t.key]||[t.key];byTierAll[t.key]=perks.filter(p=>show.includes(p.elite_tier))});
const byTier={};TIERS.forEach(t=>{const show=tierUp[t.key]||[t.key];byTier[t.key]=filteredPerks.filter(p=>show.includes(p.elite_tier))});
const perkCatCounts={};CATS.forEach(c=>{perkCatCounts[c.key]=0});perks.forEach(p=>{if(perkCatCounts[p.category]!==undefined)perkCatCounts[p.category]+=1});
const tr=perks.length,catC=new Set(perks.map(p=>p.category)).size,score=pscore(tr,catC);
...
<div style={{background:"#0f172a",borderRadius:10,padding:28,border:"1px solid #1e293b",marginBottom:24}}>
{sf&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h3 style={{fontSize:20,fontWeight:700,color:"#fff",fontFamily:FD,margin:0}}>{editId?"Edit Perk":"Report Your Stay"}</h3><button onClick={()=>{resetForm()}} style={{background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:20,fontFamily:FF,padding:"0 4px",lineHeight:1}}>×</button></div>}
{!sf&&<h3 style={{fontSize:18,fontWeight:700,color:"#fff",fontFamily:FD,marginBottom:8}}>Report Your Stay</h3>}
<p style={{fontSize:12,color:"#94a3b8",fontFamily:FF,margin:"0 0 16px"}}>Share what was provided and what was not. This directly powers the likelihood scores above.</p>
...
{(sf||editId)&&<>{entries.map((entry,i)=><div key={i} style={{background:"#111827",borderRadius:8,padding:20,border:"1px solid #334155",marginBottom:12,position:"relative"}}>
<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
<span style={{fontSize:13,fontWeight:700,color:"#e2e8f0",fontFamily:FF}}>Perk {entries.length>1?`#${i+1}`:""}</span>
{entries.length>1&&<button onClick={()=>removeEntry(i)} style={{background:"none",border:"none",color:"#fca5a5",cursor:"pointer",fontSize:18,fontFamily:FF,padding:0,lineHeight:1}} aria-label="Remove this perk">×</button>}</div>
...
{!editId&&<button onClick={addEntry} style={{background:"rgba(255,255,255,0.02)",color:"#cbd5e1",border:"2px dashed #334155",borderRadius:8,padding:"12px 20px",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:FF,width:"100%",marginBottom:16,transition:"all 0.15s"}} onMouseEnter={e=>e.currentTarget.style.borderColor="#64748b"} onMouseLeave={e=>e.currentTarget.style.borderColor="#334155"}>+ Add another category</button>}
<div style={{display:"flex",gap:8}}><button onClick={subPerk} disabled={sub} style={{...BT("#fff","#0f172a"),opacity:sub?0.5:1}}>{editId?"Save Changes":`Submit ${entries.filter(e=>e.category&&e.description.trim()).length||""} Perk${entries.filter(e=>e.category&&e.description.trim()).length!==1?"s":""}`}</button><button onClick={resetForm} style={BT("#334155","#e2e8f0")}>{editId?"Cancel Edit":"Cancel"}</button></div></>}
</div></div>
{!ld&&<div style={{background:"#fff",borderRadius:10,padding:"14px 16px",border:"1px solid #e2e8f0",marginBottom:16}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:8}}><span style={{fontSize:13,fontWeight:700,color:"#0f172a",fontFamily:FF}}>Filter Reports By Perk Category</span><span style={{fontSize:10,color:"#64748b",fontFamily:FF,textTransform:"uppercase",letterSpacing:0.8}}>Tap chips to narrow Ambassador/Titanium/Platinum sections</span></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><button onClick={()=>setPerkCategoryFilter([])} style={{background:perkCategoryFilter.length===0?"#0f172a":"#fff",color:perkCategoryFilter.length===0?"#fff":"#334155",border:perkCategoryFilter.length===0?"1px solid #0f172a":"1px solid #e2e8f0",borderRadius:999,padding:"6px 10px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:FF}}>All ({perks.length})</button>{CATS.map(c=>{const active=perkCategoryFilter.includes(c.key),count=perkCatCounts[c.key]||0;return<button key={c.key} onClick={()=>setPerkCategoryFilter(prev=>prev.includes(c.key)?prev.filter(k=>k!==c.key):[...prev,c.key])} style={{background:active?"#0f172a":"#fff",color:active?"#fff":"#334155",border:active?"1px solid #0f172a":"1px solid #e2e8f0",borderRadius:999,padding:"6px 10px",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:FF,opacity:count===0?0.55:1}}>{c.icon} {c.label} ({count})</button>})}</div>{perkCategoryFilter.length>0&&<div style={{marginTop:8,fontSize:11,color:"#64748b",fontFamily:FF}}>Showing {filteredPerks.length} of {perks.length} reports in selected categories. This includes whether the perk was provided (e.g., breakfast complimentary/credit/not included).</div>}</div>}
...
{perkCategoryFilter.length>0&&filteredPerks.length===0&&<div style={{background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:8,padding:"10px 12px",marginBottom:12,fontSize:12,color:"#9a3412",fontFamily:FF}}>No reports match the selected perk categories for this hotel yet. Try clearing or changing filters.</div>}
...
<div style={{fontSize:12,color:"#64748b",fontFamily:FF,display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginTop:14,paddingTop:12,borderTop:"1px solid #f1f5f9"}}><div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}><span style={{color:"#0f172a",fontWeight:700,fontSize:15}}>{c}</span> perk reports{hotel.room_count&&<span style={{fontSize:10,color:"#94a3b8",whiteSpace:"nowrap"}}>{hotel.room_count} rooms</span>}</div>{user&&onToggleFollow&&<button onClick={e=>{e.stopPropagation();onToggleFollow(hotel.id,followed)}} disabled={busy} style={{background:followed?"#e2e8f0":"#f8fafc",border:"1px solid #e2e8f0",borderRadius:4,padding:"2px 8px",fontSize:10,fontWeight:700,color:followed?"#334155":"#475569",fontFamily:FF,cursor:"pointer",opacity:busy?0.65:1,whiteSpace:"nowrap"}}>{followed?"Following":"Follow"}</button>}</div></div>}
...
function AdminHotelEditor({user,isAdmin,onClose,onNeedAuth,onHotelsUpdated,onOpenHotel}){
const[query,setQuery]=useState(""),[selectedId,setSelectedId]=useState(""),[form,setForm]=useState({}),[saving,setSaving]=useState(false),[adminHotels,setAdminHotels]=useState([]),[ld,setLd]=useState(true),[err,setErr]=useState("");
const loadAll=useCallback(async()=>{if(!user||!isAdmin){setAdminHotels([]);setLd(false);return}setLd(true);setErr("");try{let all=[];let from=0;const step=1000;while(true){const{data,error}=await supabase.from("hotels").select("*").order("name").range(from,from+step-1);if(error)throw error;if(!data||!data.length)break;all=all.concat(data);if(data.length<step)break;from+=step}setAdminHotels(all)}catch(e){setErr("Failed to load hotels for editing.");console.error(e)}setLd(false)},[user,isAdmin]);
useEffect(()=>{loadAll()},[loadAll]);
const hasOwn=(row,key)=>Object.prototype.hasOwnProperty.call(row||{},key);
const filtered=useMemo(()=>{const q=query.trim().toLowerCase();const base=(adminHotels||[]).filter(h=>{if(!q)return true;const hay=[h.name,h.brand,h.location,h.slug,h.city,h.state,h.country,h.region,h.marriott_code].filter(Boolean).join(" ").toLowerCase();return hay.includes(q)});return base.slice(0,300)},[adminHotels,query]);
useEffect(()=>{if(!selectedId&&filtered.length)setSelectedId(filtered[0].id);if(selectedId&&!filtered.some(h=>h.id===selectedId))setSelectedId(filtered[0]?.id||"")},[filtered,selectedId]);
const selected=useMemo(()=>adminHotels.find(h=>h.id===selectedId)||null,[adminHotels,selectedId]);
const editableFields=useMemo(()=>{if(!selected)return[];return[
...
{key:"marriott_code",label:"Marriott Code"},
{key:"website",label:"Website URL"},
{key:"email",label:"Hotel Email"},
{key:"phone",label:"Phone"},
{key:"room_count",label:"Room Count",type:"number"},
].filter(f=>hasOwn(selected,f.key))},[selected]);
const mapRowToForm=useCallback(row=>{if(!row)return{};const next={};editableFields.forEach(f=>{const v=row[f.key];next[f.key]=v===null||v===undefined?"":String(v)});return next},[editableFields]);
useEffect(()=>{if(selected)setForm(mapRowToForm(selected));else setForm({})},[selectedId,mapRowToForm,selected]);
const setField=(k,v)=>setForm(p=>({...p,[k]:v}));
const reset=()=>{if(selected)setForm(mapRowToForm(selected))};
const save=async()=>{if(!selected||saving)return;const payload={};for(const f of editableFields){const key=f.key;let raw=form[key]??"";if(f.type==="number"){if(String(raw).trim()===""){payload[key]=null;continue}const n=parseInt(String(raw).replace(/[^\d]/g,""),10);if(Number.isNaN(n)){showToast(`${f.label} must be a number`,"error");return}payload[key]=n;continue}
raw=String(raw).trim();if(key==="slug"){raw=mkSlug(raw||form.name||selected.name||"")}if(key==="marriott_code")raw=raw.toUpperCase();if(key==="website"&&raw&&!/^https?:\/\/\S+$/i.test(raw)){showToast("Website must be a valid URL (https://...)","error");return}if(key==="email"&&raw&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(raw)){showToast("Hotel email looks invalid","error");return}if(raw&&key!=="website"&&key!=="email")raw=sanitize(raw);if(f.required&&!raw){showToast(`${f.label} is required`,"error");return}payload[key]=raw||null}
setSaving(true);const{data,error}=await supabase.from("hotels").update(payload).eq("id",selected.id).select("*").single();setSaving(false);
if(error){showToast("Save failed: "+error.message,"error");return}
showToast("Hotel profile updated.");
if(data)setForm(mapRowToForm(data));
await loadAll();
if(onHotelsUpdated)await onHotelsUpdated();
};
if(!user)return<div style={{textAlign:"center",padding:60}}><h3 style={{fontSize:20,fontWeight:700,color:"#0f172a",fontFamily:FD,marginBottom:8}}>Admin Hotel Editor</h3><p style={{fontSize:13,color:"#64748b",fontFamily:FF,marginBottom:18}}>Sign in to edit hotel profiles.</p><button onClick={onNeedAuth} style={BT()}>Sign In</button><button onClick={onClose} style={{...BT("#e2e8f0","#64748b"),marginLeft:8}}>← Back</button></div>;
if(!isAdmin)return<div style={{textAlign:"center",padding:60}}><h3 style={{fontSize:20,fontWeight:700,color:"#0f172a",fontFamily:FD,marginBottom:8}}>Access denied</h3><p style={{fontSize:13,color:"#64748b",fontFamily:FF,marginBottom:18}}>Your account is not in the admin list.</p><button onClick={onClose} style={BT("#e2e8f0","#64748b")}>← Back</button></div>;
return<div><button onClick={onClose} style={{background:"#fff",border:"1px solid #e2e8f0",cursor:"pointer",fontSize:13,color:"#334155",fontWeight:600,fontFamily:FF,padding:"8px 16px",marginBottom:24,borderRadius:6}}>← Back</button>
<div style={{background:"#0f172a",borderRadius:12,padding:36,marginBottom:20,color:"#fff"}}><p style={{fontSize:10,fontWeight:700,color:"#94a3b8",textTransform:"uppercase",letterSpacing:2,marginBottom:8,fontFamily:FF}}>Admin</p><h1 style={{fontSize:34,fontWeight:700,margin:"0 0 6px",fontFamily:FD}}>Edit Hotel Profiles</h1><p style={{fontSize:14,color:"#94a3b8",fontFamily:FF,margin:0}}>Fix names, capitalization, slugs, and hotel metadata directly.</p></div>
{err&&<div role="alert" style={{background:"#fef2f2",color:"#dc2626",padding:"10px 14px",borderRadius:8,fontSize:12,fontFamily:FF,marginBottom:12}}>{err}</div>}
<div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:12}}>
<div style={{flex:"1 1 280px"}}><label style={LS}>Find Hotel</label><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search by hotel, location, slug, or Marriott code..." style={IS}/></div>
<div style={{display:"flex",alignItems:"flex-end",gap:8}}><button onClick={()=>setQuery("")} style={BT("#fff","#334155")}>Clear</button><button onClick={loadAll} style={BT("#fff","#334155")}>Refresh</button><span style={{fontSize:11,color:"#94a3b8",fontFamily:FF,paddingBottom:10}}>{filtered.length.toLocaleString()} shown</span></div>
</div>
{ld?<div style={{padding:20}}><Skeleton h={100} r={8}/><div style={{height:10}}/><Skeleton h={100} r={8}/></div>:<div style={{display:"grid",gridTemplateColumns:"minmax(260px,320px) minmax(0,1fr)",gap:12}}>
<div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,maxHeight:560,overflowY:"auto"}}>{filtered.map(h=>{const active=h.id===selectedId;return<button key={h.id} onClick={()=>setSelectedId(h.id)} style={{display:"block",width:"100%",textAlign:"left",padding:"10px 12px",border:"none",borderBottom:"1px solid #f1f5f9",background:active?"#eff6ff":"#fff",cursor:"pointer"}}><div style={{fontSize:12,fontWeight:700,color:"#0f172a",fontFamily:FF,lineHeight:1.3}}>{h.name}</div><div style={{fontSize:10,color:"#64748b",fontFamily:FF,marginTop:3}}>{[h.brand,h.location].filter(Boolean).join(" · ")}</div><div style={{fontSize:10,color:"#94a3b8",fontFamily:FF,marginTop:2}}>{h.slug}</div></button>})}{!filtered.length&&<div style={{padding:18,fontSize:12,color:"#94a3b8",fontFamily:FF}}>No hotels match your search.</div>}</div>
<div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,padding:18}}>{selected?<><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap",marginBottom:12}}><div><div style={{fontSize:17,fontWeight:700,color:"#0f172a",fontFamily:FD,lineHeight:1.25}}>{selected.name}</div><div style={{fontSize:11,color:"#94a3b8",fontFamily:FF,marginTop:4}}>ID {selected.id.slice(0,8)}... · slug {selected.slug}</div></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{onOpenHotel&&<button onClick={()=>onOpenHotel(selected)} style={BT("#fff","#334155")}>Open Hotel Page</button>}<button onClick={reset} style={BT("#fff","#334155")}>Reset</button></div></div>
<div style={{display:"flex",gap:12,flexWrap:"wrap"}}>{editableFields.map(f=><div key={f.key} style={{flex:f.type==="number"?"1 1 160px":"1 1 260px"}}><label style={LS}>{f.label}{f.required&&<span style={{fontWeight:400,textTransform:"none"}}> (required)</span>}</label><input value={form[f.key]??""} onChange={e=>setField(f.key,e.target.value)} style={IS} inputMode={f.type==="number"?"numeric":"text"} placeholder={f.key==="slug"?"hotel-name-slug":""}/></div>)}</div>
<div style={{display:"flex",gap:8,marginTop:16}}><button onClick={save} disabled={saving} style={{...BT(),opacity:saving?0.6:1}}>{saving?"Saving...":"Save Hotel"}</button></div>
</>:<div style={{fontSize:12,color:"#94a3b8",fontFamily:FF}}>Select a hotel to edit.</div>}</div>
</div>
}
</div>}
...
<style>{`::selection{background:#0f172a;color:#fff}input::placeholder,textarea::placeholder{color:#94a3b8}body{margin:0}html,body,#root{max-width:100%;overflow-x:hidden}
...
@media(max-width:768px){.ps-header{flex-direction:column!important;align-items:flex-start!important}.ps-nav{order:1;width:100%;justify-content:flex-start!important;flex-wrap:wrap!important}.ps-auth{order:2;width:100%;margin-top:6px;display:flex;flex-wrap:wrap;justify-content:flex-start}.ps-detail-layout{flex-direction:column!important}.ps-tips-sidebar{flex:1 1 auto!important;position:static!important;width:100%!important;max-width:100%!important}.ps-stats>div{flex-direction:column!important;gap:2px!important}}`}</style>
<Toaster/>
...
<div style={{background:"#0f172a",padding:isHome?"0 0 56px":"0 0 20px"}}><div style={{maxWidth:1100,margin:"0 auto",padding:"16px 28px 0",width:"100%",boxSizing:"border-box"}}>
...
<nav className="ps-nav" style={{display:"flex",alignItems:"center",gap:2}} aria-label="Main navigation">{navBtn("Compare","/compare")}{navBtn("Leaderboard","/leaderboard")}{user&&navBtn("Following","/following")}{isAdmin&&navBtn("Admin","/admin/requests")}{isAdmin&&navBtn("Edit Hotels","/admin/hotels")}</nav>
