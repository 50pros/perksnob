...
{[{l:"Home",p:"/"},{l:"Search Perks",p:"/search"},{l:"Compare",p:"/compare"},{l:"Leaderboard",p:"/leaderboard"}].map(x=><a key={x.p} href={x.p} onClick={e=>{e.preventDefault();window.history.pushState({},"",x.p);window.dispatchEvent(new PopStateEvent("popstate"))}} style={{display:"block",fontSize:13,color:"#64748b",fontFamily:FF,textDecoration:"none",marginBottom:6,transition:"color 0.15s"}} onMouseEnter={e=>e.target.style.color="#fff"} onMouseLeave={e=>e.target.style.color="#64748b"}>{x.l}</a>)}</div>
...
function HotelDetail({hotel,user,onBack,onNeedAuth,allHotels,perkCounts,onSelectHotel}){const[sf,ssf]=useState(false),[perks,sp]=useState([]),[cmts,sc]=useState([]),[ld,sl]=useState(true),[err,ser]=useState("");
...
export default function App(){const[path,nav]=usePath();const[user,su]=useState(null),[hotels,sh]=useState([]),[pc,spc]=useState({}),[scores,ssc]=useState({}),[search,ss]=useState(""),[bf,sbf]=useState(""),[showAuth,ssa]=useState(false),[showAdd,ssad]=useState(false),[profId,spid]=useState(null),[ld,sld]=useState(true),[showAll,setSA]=useState(false),[perkFilter,setPF]=useState([]),[hotelPerks,setHP]=useState({});
...
<nav className="ps-nav" style={{display:"flex",alignItems:"center",gap:2}} aria-label="Main navigation">{navBtn("Compare","/compare")}{navBtn("Leaderboard","/leaderboard")}</nav>
...
{Object.keys(pc).length>0&&<div style={{display:"flex",gap:24,marginBottom:28}}><div><span style={{fontSize:28,fontWeight:700,color:"#fff",fontFamily:FD}}>{Object.values(pc).reduce((a,b)=>a+b,0).toLocaleString()}</span><span style={{fontSize:12,color:"#94a3b8",fontFamily:FF,marginLeft:8}}>perk reports</span></div><div><span style={{fontSize:28,fontWeight:700,color:"#fff",fontFamily:FD}}>{Object.keys(pc).length.toLocaleString()}</span><span style={{fontSize:12,color:"#94a3b8",fontFamily:FF,marginLeft:8}}>hotels with data</span></div><div><span style={{fontSize:28,fontWeight:700,color:"#fff",fontFamily:FD}}>{hotels.length.toLocaleString()}</span><span style={{fontSize:12,color:"#94a3b8",fontFamily:FF,marginLeft:8}}>properties</span></div></div>}
...
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:10}}>{cSlice.map(h=><HotelCard key={h.id} hotel={h} perkCounts={pc} score={scores[h.id]||0} onClick={()=>openHotel(h)}/>)}</div>
...
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:10}}>{bSlice.map(h=><HotelCard key={h.id} hotel={h} perkCounts={pc} score={scores[h.id]||0} onClick={()=>openHotel(h)}/>)}</div>
