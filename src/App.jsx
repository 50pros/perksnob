...
const page=path==="/leaderboard"?"leaderboard":path==="/search"?"search":path==="/compare"?"compare":path==="/admin/requests"?"admin_requests":path.startsWith("/profile/")?"profile":citySlug?"city":brandSlug?"brand":hotelSlug?"hotel":"home";

const pageTitle=page==="hotel"&&sel?`${sel.name} — Elite Perk Benefits | PerkSnob`:page==="city"&&citySlug?`Marriott Hotels in ${citySlug.replace(/-/g," ").replace(/\b\w/g,c=>c.toUpperCase())} | PerkSnob`:page==="brand"&&brandSlug?`${brandSlug.replace(/-/g," ").replace(/\b\w/g,c=>c.toUpperCase())} Hotels | PerkSnob`:page==="leaderboard"?"Leaderboard | PerkSnob":page==="search"?"Search Perks | PerkSnob":page==="compare"?"Compare Hotels | PerkSnob":page==="admin_requests"?"Admin Requests | PerkSnob":"PerkSnob — Marriott Elite Benefits, Crowdsourced";
useTitle(pageTitle);
...
<nav className="ps-nav" style={{display:"flex",alignItems:"center",gap:2}} aria-label="Main navigation">{navBtn("Compare","/compare")}{navBtn("Leaderboard","/leaderboard")}{isAdmin&&navBtn("Admin","/admin/requests")}</nav>
